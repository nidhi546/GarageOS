...
export const ManagerJobsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const [activeFilter, setActiveFilter] = useState<JobCardStatus | 'ALL'>('ALL');
  ...
  const filtered = jobCards.filter(j => {
    ...
    const matchSearch = !search
      ...
      || (j.vehicle as any)?.registration_number?.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topBar}>
        <SearchBar value={search} onChangeText={setSearch} placeholder="Search by customer or plate..." />
      </View>
      <View style={styles.filters}>
        {FILTERS.map(f => (
          <TouchableOpacity
            key={f.value}
            style={[styles.filterChip, activeFilter === f.value && styles.filterActive]}
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          keyExtractor={i => i.id}
          renderItem={({ item }) => (
            <JobCardListItem jobCard={item} onPress={() => navigation.navigate('JobCardDetail', { id: item.id })} />
          )}
          contentContainerStyle={styles.list}
          ListEmptyComponent={<EmptyState title="No jobs found" message="Try adjusting your filters" icon="construct-outline" />}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  filterChip: { paddingHorizontal: SPACING.sm, paddingVertical: 6, borderRadius: RADIUS.full, backgroundColor: COLORS.surface, borderWidth: 1.5, borderColor: COLORS.border },
  filterActive: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  filterText: { fontSize: FONT.sizes.xs, fontWeight: '600', color: COLORS.textSecondary },
